...
public class CollisionHandler {
	...
	public void CheckCollisions(List<Entity> entities) {
		...
		if (gameLoop.keyHandler.buttonPriorities.isEmpty()) {
			...
			// Then, based on the direction, we need to check for the two grid squares that
			// the character occupies in the given direction.

			if (this.direction == "W" || this.direction == "S") { // if the direction was vertical, we need to check the left
				...
				if (character.posX % 48 <= 19) {
					System.out.println("in block");
				} // TODO usarlo per poter passare in mezzo ai blocchi.
